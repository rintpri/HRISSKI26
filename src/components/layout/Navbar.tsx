  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur-md sm:px-6">
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        {/* Fast Role Simulator Switcher - Hanya untuk Admin / HRD */}
        {currentEmployee.role === 'hrd' ? (
          <div className="relative">
            <button
              onClick={() => setShowRoleMenu(!showRoleMenu)}
              className="flex items-center gap-2 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-xs font-medium text-indigo-900 hover:bg-indigo-100 transition shadow-sm"
              title="Ganti Mode Peran Pengguna (Fitur Khusus Admin/HRD)"
            >
          ...
          <div className="hidden sm:flex items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs text-slate-600">
            <span className="text-[11px] text-slate-400">Peran:</span>
            <span className={`rounded px-1.5 py-0.5 text-[10px] font-bold ${roleLabels[currentRole].color}`}>
              {roleLabels[currentRole].name}
            </span>
          </div>
        )}
              ...
              {currentEmployee.role !== 'hrd' && (
                <div className="mt-3 pt-2 border-t border-slate-100 flex justify-end">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 py-2 text-xs font-bold text-white transition"
                  >
                    Keluar (Logout)
                  </button>
                </div>
